...
10. PR Review: Early feedback may happen on a Draft PR. After the PR is Ready for Review, formal review must use the `deep-review` skill. `/review` remains as a lightweight fallback. **Reviewer Agents must report every finding with severity + confidence, not pre-filter by importance** — Opus 4.7 follows "only report high-severity" type instructions literally, which lowers recall on real bugs; let the human do the filtering.
