...
8. Parallel Implementation: When the green-phase implementation naturally splits across multiple independent files, invoke the `parallel-implementation` skill — it returns a slice plan (file assignments, dependencies, per-slice output-format contracts); dispatch accordingly with parallel `Agent` calls + `isolation: "worktree"` per plan.
