...
**ship-specific narrowing of "closed out"**: blocking findings from the `deep-review` skill's **Spec Conformance** reviewer cannot be deferred to a follow-up issue. Unlike `auto` / `step` modes, ship has no human checkpoint between the spec and PR Ready — a misinterpreted acceptance criterion or uncaught scope creep can't surface downstream. ship's contract is spec → Ready in this single PR; deferring a Spec Conformance blocker violates that contract. If it can't be fixed in the current iteration, take the Blocked exit instead.

**Unavoidable-enabler scope** (ship-specific escape): when the Agent must implement scope outside the spec — typically because the spec requires X but X depends on an unmentioned Y — document the addition in the PR comment's `自主决定 / Autonomous decisions` section. A documented enabler is justified scope; undocumented additions are unjustified scope creep and remain blocking under the skill's default classification rule.
